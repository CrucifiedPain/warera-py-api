...
def test_scope_reset_across_context_does_not_raise() -> None:
    """
    Entering a scope in one thread/context and exiting in another must not raise
    ValueError from ContextVar.reset across Contexts.
    """
    scope = CancellationScope()
    ...
    # Enter in the current context, then run enter/exit in a fresh thread too.
    with scope:
        pass

    t = threading.Thread(target=enter_and_exit)
    t.start()
    t.join(timeout=2.0)
    assert not errors, f"reset raised across contexts: {errors}"


def test_scope_reusable_after_cancel() -> None:
    """After cancel(), re-entering the scope clears the cancelled flag."""
    scope = CancellationScope()
    with scope:
        scope.cancel()
        assert scope.is_cancelled is True
    # Re-enter: flag must reset so future requests aren't auto-cancelled.
    with scope:
