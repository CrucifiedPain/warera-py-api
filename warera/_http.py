...
def _get_rt_header(status_codes: list[int] | None = None) -> str:
    default_retryable_status_codes = (
        sorted(status_codes) if status_codes is not None else DEFAULT_RETRYABLE_STATUS_CODES
    )
...
class _RateLimitState:
    ...
    def consume(self) -> None:
        """
        Optimistically decrement the local ``remaining`` counter before a
        request is dispatched. Without this, a burst of concurrent requests all
        read the same stale ``remaining > 0`` and none waits, letting the client
        blow past the server window before any response updates the counter (H1).
        """
    ...
    def update(self, headers: httpx.Headers) -> None:
        ...
        # Detect a window refresh via the reset timestamp, not via a rising
        # `remaining` — concurrent responses arrive out of order, so a stale
        # higher `remaining` must not be mistaken for a fresh window (H2).
        new_reset_at: float | None = None
        if raw_reset is not None:
            with contextlib.suppress(ValueError):
                new_reset_at = time.monotonic() + float(raw_reset)

        window_refreshed = (
            new_reset_at is not None
            and self._reset_at is not None
            and new_reset_at > self._reset_at + 1.0
        )

        if raw_remaining is not None:
            with contextlib.suppress(ValueError):
                new_remaining = int(raw_remaining)
                if self.remaining is None or window_refreshed:
                    if window_refreshed:
                        self.window_refreshes += 1
                    self.remaining = new_remaining
                else:
                    # Same window: quota only decreases. Ignore out-of-order
                    # responses reporting a higher value than we already know.
                    self.remaining = min(self.remaining, new_remaining)
                self._prev_remaining = self.remaining

        if new_reset_at is not None and (self._reset_at is None or window_refreshed):
...
class HttpSession:
    ...
    def __init__(
        ...
        # Advertise the SAME retryable set to the server via the `rt` header so
        # it matches this client's actual retry behaviour (H5).
        self._rt_header = _get_rt_header(list(self._retryable_status_codes))
    ...
    async def aclose(self) -> None:
        # Cancel the pending auto-batch flush and await it so it doesn't linger
        # as a "Task was destroyed but it is pending" warning (H4).
        if self._auto_batch_task and not self._auto_batch_task.done():
            self._auto_batch_task.cancel()
            with contextlib.suppress(asyncio.CancelledError):
                await self._auto_batch_task
        self._auto_batch_task = None

        # Fail any futures still queued but never flushed, so callers awaiting
        # get() don't hang forever.
        pending, self._auto_batch_queue = self._auto_batch_queue, []
        for _priority, _proc, _params, fut in pending:
            if not fut.done():
                fut.set_exception(WareraError("Client closed before request was sent"))

        # Cancel any in-flight SWR revalidation tasks.
        with contextlib.suppress(Exception):
            self._swr_cache.clear()
    ...
    def _make_before_sleep(self) -> Callable[[Any], None]:
        ...
        fire the telemetry ``on_retry`` hook, and additionally invoke the user's
        ``on_retry`` callback when set.
        """
        ...
        def combined(retry_state: Any) -> None:
            ...
            if telemetry is not None:
                telemetry_hook = getattr(telemetry, "on_retry", None)
                if callable(telemetry_hook):
                    try:
                        telemetry_hook(retry_state.attempt_number, delay)
                    except Exception:
                        logger.exception("telemetry.on_retry raised; ignoring")
    ...
    def _retrying(self) -> AsyncRetrying:
        ...
        exp_wait = (wait_random_exponential if self._jitter else wait_exponential)(
        ...
        def wait_strategy(retry_state: Any) -> float:
            # Honour the server's own timing on rate-limit errors instead of a
            # blind exponential schedule (H3): prefer Retry-After, then the
            # reset window, capped by max_delay. Fall back to exponential.
            exc = retry_state.outcome.exception() if retry_state.outcome else None
            server_delay: float | None = None
            if isinstance(exc, WareraRateLimitError) and exc.retry_after is not None:
                server_delay = float(exc.retry_after)
            elif self._rate_limit._reset_at is not None:
                remaining_secs = self._rate_limit._reset_at - time.monotonic()
                if remaining_secs > 0:
                    server_delay = remaining_secs
            if server_delay is not None:
                return min(max(0.0, server_delay), max_delay)
    ...
    async def _auto_batch_flush(self) -> None:
        ...
        # Abort the shared in-flight request only when EVERY caller has cancelled
        # its future (e.g. scope.cancel() cancelled them all). If some callers
        # still want results, the request keeps running.
        flush_task = asyncio.current_task()

        def _on_fut_cancel(_f: asyncio.Future[Any]) -> None:
            if flush_task and not flush_task.done() and all(f.cancelled() for f in futs):
                flush_task.cancel()

        for f in futs:
            f.add_done_callback(_on_fut_cancel)

        try:
            results = await self.post_batch(
                procedures, inputs, max_batch_size=self._max_batch_size
            )
            ...
            for f in futs:
                f.remove_done_callback(_on_fut_cancel)
            if self._telemetry:
                exec_time_ms = (time.time() - start_time) * 1000
