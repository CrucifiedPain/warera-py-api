...
class RequestPriority(StrEnum):  # type: ignore[misc]
    ...
    HIGH wakes the auto-batch window early so the request is flushed on the next
    tick instead of waiting the full debounce delay.
    NORMAL and LOW both wait for the standard auto-batch window; within a flush,
    items are ordered HIGH → NORMAL → LOW before being sent.
    """
    HIGH = "high"
    NORMAL = "normal"
    LOW = "low"
