...
## Migrating to v0.2.3 (from v0.2.2)
...
5. **Built-in Telemetry Hooks**: You can pass a `TelemetryHooks` object to observe internal rate-limit sleep durations, chunk payload efficiencies, and cache hit ratios for Datadog/Prometheus tracking. Telemetry also receives an `on_retry(attempt, delay_seconds)` event.

### What to know
The new feature entry points above are the whole public surface for `0.2.3`. A couple of things worth calling out:

- **`warera.Battle` is importable** from the top-level package, and **`client.war.get(...)`** works on both the async and sync clients (the `War` model parses the API's `_id` field, and the sync client now exposes `war`).
- **`search.search_mus` / `search.search_users`** take the correct `searchText` parameter and return parsed `SearchResult` lists.
- **A few convenience methods aren't part of `0.2.3`.** Each duplicated an existing, correct method, so use the canonical method instead:
